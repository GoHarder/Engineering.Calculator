...
lib.sendResetEmail = async (to, data) => {
   ...
   const subject = 'Engineering Calculator | Password Reset Verification Code';
...
lib.sendNewUserEmail = async (to, data) => {
   const { from: configFrom, sender: configSender } = config.mailGun;

   const from = `no-reply(via Eng Calc) <${configFrom}>`;
   const sender = `no-reply(via Eng Calc) <${configSender}>`;
   const subject = 'Engineering Calculator | System Registration';
   const html = lib.loadTemplate('new-user', data);
   const replyTo = undefined;
...
lib.sendSharedWorkbookEmail = async (to, data) => {
   const { from: configFrom, sender: configSender } = config.mailGun;

   const from = `no-reply(via Eng Calc) <${configFrom}>`;
   const sender = `no-reply(via Eng Calc) <${configSender}>`;
   const subject = `Engineering Calculator | Shared Workbook (${data.title})`;
   const html = lib.loadTemplate('share-workbook', data);
   const replyTo = undefined;
