...
lib.deleteOldTokens = () => {
   // Get all the tokens in the system
   const tokens = _data.list('tokens');

   if (tokens) {
      tokens.forEach((token) => {
         // Read the token
         const tokenData = _data.read('tokens', token);
...
/** Run the background workers once per hour */
lib.loop = () => {
   setInterval(() => {
      lib.deleteBadTokens();
   }, 1000 * 60 * 60);
