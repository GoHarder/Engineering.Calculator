...
// TODO: 7-12-2021 3:27 PM - Optional: If null shows up fix
 ...
 */
lib.postBeacon = (data, callback) => {
   mongo.app.collection('proj').updateMany({ checkout: mongo.ObjectID(data.payload._id) }, { $unset: { checkout: 1 } }, (error, data) => {
      if (!error && data) {
         // console.log(data);
      } else {
         console.log(error);
...
lib.get = (data, callback) => {
   ...
   } else if (data.query.customer) {
      lib.getCustomer(data, callback);
   } else if (data.dir) {
...
lib.getByCalcId = (data, callback) => {
   ...
   if (data.headers.clean !== 'none') {
      mongo.app.collection('proj').updateOne({ _id: mongo.ObjectID(data.headers.clean) }, { $unset: { checkout: 1 } });
   }
...
// TODO: 7-07-2021 1:57 PM - Document method
lib.getCustomer = async (data, callback) => {
   const search = data.query.customer;

   const pipeline = [{ $search: { autocomplete: { query: search, path: 'customer' } } }, { $project: { _id: 0, customer: 1 } }];

   let customers = await mongo.app.collection('proj').aggregate(pipeline).toArray();

   // TODO: 7-13-2021 9:31 AM - You can add a limit to the length of customers returned within the pipeline
   // Add a set timeout

   customers = customers.map((doc) => doc.customer);

   callback(200, customers);
};
...
lib.updateProject = (data, callback) => {
   ...
   data.payload.creator._id = mongo.ObjectID(data.payload.creator._id);
   ...
   // Clean up the dates
   data.payload.created = new Date(data.payload.created);
   data.payload.opened.map((entry) => (entry.time = new Date(entry.time)));
   ...
   if (token.id === `${data.payload.checkout}`) {
      mongo.app.collection('proj').updateOne({ _id: data.payload._id }, { $set: data.payload }, (error, info) => {
         if (!error && info) {
            ...
            // console.log(error);
            callback(500, { error: { workbook: 'Could not save project (292)' } });
         }
      });
   } else {
      mongo.app.collection('proj').findOne({ _id: data.payload._id }, (error, doc) => {
         if (!error && doc) {
            ...
            callback(500, { error: { workbook: 'Could not save project (300)' } });
         }
      });
   }
};
