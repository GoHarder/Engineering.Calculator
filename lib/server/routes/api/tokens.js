...
lib.delete = (data, callback) => {
   // Validate the id
   const tokenId = parse(data.dir).root;

   const test = validate.string(tokenId);
   ...
   if (test) {
      ...
      if (tokenData) {
         const unlinked = _data.delete('tokens', tokenId);
...
/**
 * Verifies if the token belongs to a user
 * @param {string} userId The user id
 * @param {string} tokenId The token id
 */
lib.validate = (userId, tokenId) => {
   let output = false;

   const token = _data.read('tokens', tokenId);

   if (token) {
      if (userId === token.userId && token.expires > Date.now()) {
         output = true;
      }
   }
