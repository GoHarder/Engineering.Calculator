      ...
      mongo.app.collection('users').findOne({ email }, (error, userDoc) => {
         if (!error && userDoc) {
            ...
            if (hashedPassword === userDoc.hashedPassword && email == userDoc.email) {
               ...
               const token = jwt.sign(payload, config.key.jwt, { noTimestamp: true });

               callback(200, { token });
            } else {
               callback(403, { error: { password: 'Password is incorrect' } });
            }
         } else {
            callback(500, { error: { email: 'Could not find user' } });
         }
      });
   } else {
