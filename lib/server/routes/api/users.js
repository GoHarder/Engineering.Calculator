...
const { password } = require('../../../validate');
...
lib.getWithToken = (data, callback) => {
   ...
   if (token) {
      // Lookup the user
      mongo.app.users.findOne({ _id: mongo.ObjectID(token.id) }, { projection: { hashedPassword: 0 } }, (error, data) => {
         if (!error && data) {
            data.id = data._id;
            delete data._id;
...
lib.getWithEmail = (data, callback) => {
   ...
   mongo.app.users.findOne({ email }, (error, userDoc) => {
      if (!error && userDoc) {
         ...
         const sent = emailService.sendResetEmail(['gharder@hwec.com'], { resetCode });
