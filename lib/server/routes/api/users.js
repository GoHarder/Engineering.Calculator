...
lib.get = (data, callback) => {
   ...
   } else if (data.dir === '/all') {
      lib.getAll(data, callback);
   } else {
...
lib.getAll = (data, callback) => {
   mongo.app
      .collection('users')
      .find({}, { projection: { hashedPassword: 0, admin: 0 } })
      .toArray((error, docs) => {
         if (!error && docs) {
            callback(200, docs);
         } else {
 ...
 * Gets user by email
 * @param {object} data The request data
 * @param {object} data.query The search query
 * @param {string} data.query.email The user email
 * @param {function} callback The callback function
 */
lib.getWithEmail = (data, callback) => {
...
lib.put = (data, callback) => {
   ...
   if (token) {
      ...
      if (token.admin || token.id === userId) {
               ...
               if (!error && info) {
                  if (info.modifiedCount !== 0) {
                     ...
                     if (info.matchedCount === 0) {
                        callback(400, { error: { password: 'Password is incorrect' } });
                     } else {
                        callback(400, { error: { form: 'Nothing was changed' } });
                     }
                  }
               } else {
                  callback(400, { error: { form: 'A user with that email already exists' } });
               }
            });
         } else {
            callback(400, { error: test.errors });
         ...
         callback(403, { error: { form: 'Token is not valid for user data' } });
      }
   } else {
      callback(401, { error: { form: 'Authorization is invalid' } });
   }
};
