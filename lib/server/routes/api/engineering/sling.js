...
lib.root = async (data, callback) => {
   ...
   if (capacity && carSpeed && roping) {
      ...
      const _sheaveChannels = mongo.agg.getSlingSheaveChannels();
      ...
      try {
         ...
         const sheaveChannels = roping === 2 ? await mongo.engineering.collection('channel').aggregate(_sheaveChannels).toArray() : undefined;
         ...
         callback(200, { bottomChannels, models, safeties, sheaves, sheaveChannels, shoePlates, shoes, topChannels });
      } catch (error) {
         console.log(error);
