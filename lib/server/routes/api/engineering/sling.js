...
lib.root = async (data, callback) => {
   ...
   if (capacity && carSpeed) {
      ...
      try {
         ...
         const topChannels = await mongo.engineering.collection('steel').aggregate(_topChannels).toArray();
         const channels = await mongo.engineering.collection('steel').aggregate(_channels).toArray();
