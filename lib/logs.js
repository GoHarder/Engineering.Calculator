...
lib.append = (text) => {
   ...
   try {
      ...
      const fileDesc = fs.openSync(`${baseDir}/${date}.log`, 'a');

      // Append the file
      fs.appendFileSync(fileDesc, `${header()}`);
      fs.appendFileSync(fileDesc, `${text}\n\n`);

      // Close the file
      fs.closeSync(fileDesc);

      console.log('\x1b[31m%s\x1b[0m', 'Error:', `${date}.log`);
   } catch (error) {
      console.log(error);
   }
};
